...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private OnBannerLongListener  longListener;
    private DisplayMetrics        dm;
    private String mCurrentImage;
    private boolean isLongClick;
//    private boolean isFirst = true;     //防止第一次翻到最后一页张出现空白
...
//    }
            ...
            if (longListener != null) {
                view.setOnLongClickListener(new OnLongClickListener() {
                    ...
                    public boolean onLongClick (View v) {
                        isLongClick = true;
                        longListener.OnBannerLongClick(toRealPosition(position));
                        return true;
                    }
                });
            }
    ...
    public void onPageScrollStateChanged (int state) {
        ...
        Log.e(tag, "pagescroll: " + currentItem);
    ...
    public void onPageSelected (int position) {
        Log.e(tag, "1=====position: " + position + " / " + count);
//        if (isFirst && position == (count + 1))
//            isFirst = false;
        if (mOnPageChangeListener != null) {
        ...
        if (imageUrls != null && imageUrls.size() > 0) {
            if (position == 0) {
                mCurrentImage = (String) imageUrls.get(count - 1);
            } else if (position == count + 1) {
                mCurrentImage = (String) imageUrls.get(0);
            } else {
                mCurrentImage = (String) imageUrls.get(position - 1);
            }
            Log.e(tag, "===position:" + position + "==current image: " + mCurrentImage);
        }
    }
    ...
    public Banner setOnBannerLongListener (OnBannerLongListener listener) {
